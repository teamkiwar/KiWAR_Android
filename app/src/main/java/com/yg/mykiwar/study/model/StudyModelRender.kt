...
class StudyModelRender(var main: StudyModelSurfaceView) : GLSurfaceView.Renderer {
    ...
    override fun onDrawFrame(gl: GL10?) {
        ...
        var scene : StudySceneLoader? = main.parent.scene
    ...
    override fun onSurfaceChanged(gl: GL10?, width: Int, height: Int) {
        ...
        val camera = scene!!.camera
        Log.v("씬", scene!!.toString())
        Log.v("카메라", camera!!.toString())
        Matrix.setLookAtM(modelViewMatrix, 0, camera!!.xPos, camera.yPos, camera.zPos, camera.xView, camera.yView,
                camera.zView, camera.xUp, camera.yUp, camera.zUp)
